...
##' @param extraPackages Other useful R packages to be passed to the (internally crated) clusters for the parallel computations. If required, provide package names as a vector of strings.
                         ...
                         extraPackages = NULL, ...)
{
  ...
  if (m < 3) {
    ...
    objfun <- function(argv, ...){
      ...
      cfun.val <- sapply(xv, ila.dens, index = index, ...)
      ...
      log.den  <-  log(ila.dens(fullOpt$par[index], index = index, ...))
      return(log.den - log(nc.cond))
    }

    if (control$n.cores < 2) {
      message("#----------------------------------------\n cores < 2 -> computing the integral serially...\n#-----------------------------------------")
      ilaf <- aaply(.data = par.val, .margins = 2, .fun = objfun, ...)
